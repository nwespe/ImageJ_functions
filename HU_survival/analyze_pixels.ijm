macro "analyze pixels" {
	...
	// print("Values received are "+sample+" "+file);
	setBatchMode(true);
	open(file);
	...
	for (y=0; y<150; y+=145) // y vals are 0, 145
	{
	    ...
	    for (x=10; x<360; x+=115) // x vals are 10, 125, 240, 355
	    {
	        ...
	        setResult("Dilution", nResults-1, i);
	        setResult("Date", nResults-1, date);
	        setResult("Sample", nResults-1, sample);
	    ...
	    //saveAs("Jpeg", path+"_end");
	    close(); // closes time image (start or end)
	    selectWindow(main);
	}
